...
const Sidebar = ({ selectedUser, setSelectedUser }) => {
  ...
  return (
    <div
      className={`bg-[#8185B2]/10 h-full p-10 rounded-r-xl overflow-y-scroll text-white 
    ...
    >
      ...
      {/* ------------------------- Users Profile and Image --------------------------------*/}

      <div className="flex flex-col">
        {userDummyData.map((user, index) => (
          <div onClick={()=>{setSelectedUser(user)}}
            key={index}
            className={`relative items-center flex gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${
